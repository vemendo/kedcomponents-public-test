(function () {
    ...
    ***************************************************************************** */
    ...
    var scriptVars = typeof scriptVariables !== 'undefined' ? scriptVariables : undefined;

    var schoolLowerCase = resultUser.school.toLowerCase();
    var canarySchoolsLowerCase = (scriptVars.CANARYS_CHOOLS || "")
        .toLowerCase()
        .replace(/å/g, "a")
        .replace(/ä/g, "a")
        .replace(/ö/g, "o")
    ...
    var useCanary = false;
    canarySchoolsLowerCase.forEach(function (_configuredSchool) {
        var _a = __read(_configuredSchool.startsWith("!") ?
            [true, _configuredSchool.substring(1)] :
            [false, _configuredSchool], 2), negation = _a[0], configuredSchool = _a[1];
        if (configuredSchool === '*' || configuredSchool === schoolLowerCase) {
            useCanary = !negation;
        }
    });
    // CANARYS_CHOOLS=
    ...
    var KED_TOKEN_PATH = scriptVars.KED_TOKEN_PATH, KED_TOKEN_PATH_CANARY = scriptVars.KED_TOKEN_PATH_CANARY, KED_LOCALE = scriptVars.KED_LOCALE, KED_SCHOOL_LOCALE = scriptVars.KED_SCHOOL_LOCALE, KED_COURSE_BUILDER_URL = scriptVars.KED_COURSE_BUILDER_URL, KED_COURSE_VIEWER_URL = scriptVars.KED_COURSE_VIEWER_URL, KED_SUBJECT_PLANNER_ADMIN_URL = scriptVars.KED_SUBJECT_PLANNER_ADMIN_URL, KED_SUBJECT_PLANNER_URL = scriptVars.KED_SUBJECT_PLANNER_URL, KED_FEATURES = scriptVars.KED_FEATURES, KED_KEEP_ALIVE_URL = scriptVars.KED_KEEP_ALIVE_URL, KED_ENUM_WORDBANKS_URL = scriptVars.KED_ENUM_WORDBANKS_URL, KED_WORDBANKS_URL = scriptVars.KED_WORDBANKS_URL, KED_CALENDAR_PROVIDERS = scriptVars.KED_CALENDAR_PROVIDERS;
    configure({
        ...
        KED_CALENDAR_PROVIDERS: KED_CALENDAR_PROVIDERS
    });
    ...
    var devVersion = testVersion && testVersion.startsWith('dev-') ?
        ...
        null;
    var KED_CONFIGURED_CDN = scriptVars.KED_CONFIGURED_CDN, KED_CANARY_CDN = scriptVars.KED_CANARY_CDN, KED_TEST_CDN = scriptVars.KED_TEST_CDN, KED_DEV_CDN = scriptVars.KED_DEV_CDN;
    ...
    var CDN_CANARY = KED_CANARY_CDN || "https://rawgit.com/vemendo/kedcomponents-public-test/canary"; // See rollup-replace-env-vars-elements.js
    var CDN_TEST = KED_TEST_CDN || "https://rawgit.com/vemendo/kedcomponents-public-test/master"; // See rollup-replace-env-vars-elements.js
    var CDN_DEV = KED_DEV_CDN || "https://rawgit.com/vemendo/kedcomponents-public-test/dev-"; // See rollup-replace-env-vars-elements.js
